...
namespace GLTF.Schema
{
    public class Accessor : GLTFChildOfRootProperty
    {
        ...
        private static unsafe Vector3 GetVec3Element(byte[] buffer, uint byteOffset)
        {
            fixed (byte* offsetBuffer = &buffer[byteOffset])
            {
                return *((Vector3*)offsetBuffer);
            }
        }
        private static unsafe Vector4 GetVec4Element(byte[] buffer, uint byteOffset)
        {
            fixed (byte* offsetBuffer = &buffer[byteOffset])
            {
                return *((Vector4*)offsetBuffer);
            }
        }
        private static unsafe Color GetColorElement(byte[] buffer, uint byteOffset)
        {
            fixed (byte* offsetBuffer = &buffer[byteOffset])
            {
                return *((Color*)offsetBuffer);
            }
        }
...
/*
        ...
        public struct TempNumericArray
        {
            [FieldOffset(0)]
            public byte[] AsBytes;
            [FieldOffset(0)]
            ...
            */
            {
                ...
                if (ComponentType == GLTFComponentType.Float)
                {
                    if (Type == GLTFAccessorAttributeType.VEC4)
                    {
                        for (uint idx = 0; idx < Count; idx++)
                        {
                            uint elementOffset = totalByteOffset + idx * stride;
                            arr[idx] = GetColorElement(bufferViewData, elementOffset);
                            //float g = GetFloatElement(bufferViewData, elementOffset + componentSize);
                            //float b = GetFloatElement(bufferViewData, elementOffset + componentSize * 2);
                            //float a = GetFloatElement(bufferViewData, elementOffset + componentSize * 3);
                    ...
                    else
                    {
                        ...
                        {
                            uint elementOffset = totalByteOffset + idx * stride;
                            Vector3 v = GetVec3Element(bufferViewData, elementOffset);
                            //float r = GetFloatElement(bufferViewData, elementOffset);
                            //float g = GetFloatElement(bufferViewData, elementOffset + componentSize);
                            //float b = GetFloatElement(bufferViewData, elementOffset + componentSize * 2);
                            arr[idx].SetValues(v);
                        }
                ...
                else
                {
                    if (Type == GLTFAccessorAttributeType.VEC4)
                    {
                        for (uint idx = 0; idx < Count; idx++)
                        {
                            ...
                            uint elementOffset = totalByteOffset + idx * stride;
                            float r = GetDiscreteElement(bufferViewData, elementOffset, ComponentType) / maxValue;
                            float g = GetDiscreteElement(bufferViewData, elementOffset + componentSize, ComponentType) / maxValue;
                            float b = GetDiscreteElement(bufferViewData, elementOffset + componentSize * 2, ComponentType) / maxValue;
                            float a = GetDiscreteElement(bufferViewData, elementOffset + componentSize * 3, ComponentType) / maxValue;
                            arr[idx].SetValues(r, g, b, a);
                        }
                    }
                    else
                    {
